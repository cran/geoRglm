           ...
           estimator.type = c("poisson", "not-poisson"), max.dist = NULL, pairs.min = 2)
{
  ...
  mm2 <- apply(as.matrix(data1), 2, mean)^2
  sigma2 <- log(t(t((apply(as.matrix(data1), 2, var) * (n.data - 1))/n.data - apply(as.matrix(data1/units.m), 2, mean))/mm2) + 1)
...
function(x, max.dist = x$max.dist, ...)
{
  ...
  C.f <- function(x, my.l){
    if(x>0) return(cov.spatial(x, cov.model = my.l$cov.model, kappa = my.l$kappa, cov.pars = my.l$cov.pars))
    else return(my.l$nugget+cov.spatial(x, cov.model = my.l$cov.model, kappa = my.l$kappa, cov.pars = my.l$cov.pars))
  }
